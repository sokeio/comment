...
namespace Sokeio\Comment\Models\Presenters;
...
class CommentPresenter
{
    ...
    public function replaceUserMentions($text): array|string
    {
        ...
        foreach ($usernames as $username) {
            $user = (config('sokeio.model.user'))::where('name', $username)->first();

            if ($user) {
                $userRoutePrefix = config('comment.users_route_prefix', 'users');
